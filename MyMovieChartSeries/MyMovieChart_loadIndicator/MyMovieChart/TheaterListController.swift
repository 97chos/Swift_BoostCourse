...
class TheaterListController: UITableViewController {
    ...
    func callTheaterAPI() {
        ...
        let sList = 100 // 불러올 데이터 정수
        let type = "json" // 데이터 형식
        
        // 2. 인자값들을 모아 URL 객체로 선언
        ...
        do {
            ...
            do {
                ...
                let alert = UIAlertController(title: "실패",
                                              message: "데이터 분석이 실패하였습니다.", preferredStyle: .alert)
            ...
            // 경고창 형식으로 오류 메시지를 표시해준다.
            let alert = UIAlertController(title: "실패",
                                          message: "데이터 불러오기에 실패하였습니다.", preferredStyle: .alert)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        // self.list 배열에서 행에 맞는 데이터를 추출
        let obj = self.list[indexPath.row]
        
        // 재사용 큐로부터 tCell 식별자에 맞는 셀 객체를 전달
